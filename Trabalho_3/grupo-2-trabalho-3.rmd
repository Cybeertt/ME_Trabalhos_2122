...
```{r, eval = TRUE}
(tabela.2 <- table(hour.day$season,hour.day$weathersit))
dimnames(tabela.2) = list(season=c("Inverno","Primavera", "Verão", "Outono"),
                        weathersit=c("Céu Limpo","Nublado", "Aguaceiros","Tempestade"))

hourday <- chisq.test(tabela.2)
cat("\n Tabela de contigência dos dados observados\n")
...
cat("\nRegião Critica\n")
qchisq(1-alpha,hourday$parameter)

``` 
Dobs = 94.36922 e RC=[21.66599,+∞]

No nível de significância de 1% tem como 2.154533e-16 < 0.01 então rejeita-se a hipótese nula (H0).
