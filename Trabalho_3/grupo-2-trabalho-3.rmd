...
H0 − Os dados provêm da população com a existência associada entre as variáveis selecionadas.
...
muitofrio <- subset(hour, 0<=atemp & atemp<0.2)
length(muitofrio)

dimnames(mgn_my) = list(atemp=c("muito frio","frio","neutro","calor","muito calor"),
                          windspeed=c("muito vento","vento","pouco vento","neutro","sem vento"))

hourdaynight <- chisq.test(table(hour))
...
qchisq(1-alpha,hourdaynight$parameter)

hourdaynight$statistic
hourdaynight$p.value 
``` 

No nível de significância de 1% tem como 2.2e-16 < 0.01 então rejeita-se a hipótese nula (H0).

Com  base  na  amostra  e  ao  nível  de  signifiância  de 10%,  conclui-se que os tempos de falha não seguem uma existencia associada entre as variáveis selecionadas.
...
No nível de significância de 1% tem como 0.2564 > 0.01 então não se rejeita a hipótese nula (H0).

Com  base  na  amostra  e  ao  nível  de  signifiância  de 10%,  conclui-se que os tempos de falha seguem uma existencia associada entre as variáveis selecionadas.
